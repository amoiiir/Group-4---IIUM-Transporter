...
5) Model definition:
      * This model shows that the users need to choose between Student or Runner to use this web application. Then, users need to fill in their personal details in 
        registration forms that has been devided by the chosen user type by entering Name, Email, Password, Role, Vehilce Model and Plate Number. These details
        are needed so that the users can login only Email and Password.
